...
## 5b. The CITE Framework (Mnemonic)

For quick on-page audits, every section of every page should pass the CITE check. CITE is a one-pass audit you run on every H2 block during writing.

**C — Clear answer block.** Each H2 section opens with a 40 to 80 word direct answer to the H2 question. AI systems extract this block. If the answer is buried in paragraph 3, the block is failing C.

**I — Include primary-source quantitative data.** "Many patients" gets ignored. "Over 2,400 implants placed since 2015 with a documented 98.7% success rate" gets cited. Every section needs at least one verifiable number with attribution to either the practice (from the brief) or a published source (ADA, CDC, peer-reviewed dental literature).

**T — Timestamp freshness.** Every page carries a "Last Medically Reviewed" date and reviewing doctor byline. Cornerstone pages should also include a "What changed in [year]" section when content is refreshed (per the 60-day touch rule in `references/10-technical-aeo.md`). AI systems weigh recency.

**E — Entity authority.** Practice name, doctor name with credentials, city, and service must be present in the relevant section. Every section that makes a clinical claim must link the claim to the specific doctor. Anonymous claims are AI penalty signals.

A page that passes all four C-I-T-E checks at the section level is AEO-ready. Run this mentally on every section before delivery.
...
## 7b. llms.txt and llms-full.txt

llms.txt is the 2026 equivalent of robots.txt for AI systems. It tells language models how to interpret the site, which pages are authoritative, and which sections to prioritize. Deploy at site root: `/llms.txt`.

llms-full.txt is an expanded variant containing the full text of the most important pages (homepage, primary city page, top service pages, doctor pages) concatenated into one document. AI agents that support llms-full.txt can ingest the entire value proposition without crawling.

The skill generates both files automatically in Mode A (whole-site optimization). See `references/10-technical-aeo.md` for the file format specification, content requirements, and regeneration triggers.
